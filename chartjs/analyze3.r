...
mydata <- read.csv("Chart_vulnerabilities_smells_file-grain.csv")
attach(mydata)
write(sprintf("Results of vulnerabilities-smells with file grain"),"Chart_vulnerabilities-smells_summary.txt")

for (var in variables) {
	if (var == 'smelly') {
		kmsurvival <- npsurv(Surv(time, event) ~ get(var))
		jpeg('Chart_vulnerabilities_smells_file-grain_rplot.jpg')
	...
	write(sprintf("		Covariate : %s",var),"Chart_vulnerabilities-smells_summary.txt",append=TRUE)
...
detach(mydata)
mydata <- read.csv("Chart_vulnerabilities_smells_line-grain.csv")
attach(mydata)
write(sprintf("Results of vulnerabilities-smells with line grain"),"Chart_vulnerabilities-smells_summary.txt",append = TRUE)

for (var in variables) {
	if (var == 'smelly') {
		kmsurvival <- npsurv(Surv(time, event) ~ get(var))
		jpeg('Chart_vulnerabilities_smells_line-grain_rplot.jpg')
		plot(kmsurvival,xlab = "Time (in hours)", ylab = "Survival Probability",lty = c(1,3),xaxs="r")
		legend("bottomleft", legend=c("NotSmelly", "Smelly"),lty=c(1,3),cex=1.3)
		dev.off()
		ev <- "event"
...
detach(mydata)
mydata <- read.csv("Chart_vulnerabilities_smells_line-grain_large.csv")
attach(mydata)
write(sprintf("Results of vulnerabilities-smells with line grain and considering dependencies"),"Chart_vulnerabilities-smells_summary.txt",append = TRUE)

for (var in variables) {
	if (var == 'smelly') {
		kmsurvival <- npsurv(Surv(time, event) ~ get(var))
		jpeg('Chart_vulnerabilities_smells_line-grain_large_rplot.jpg')
		plot(kmsurvival,xlab = "Time (in hours)", ylab = "Survival Probability",lty = c(1,3),xaxs="r")
		legend("bottomleft", legend=c("NotSmelly", "Smelly"),lty=c(1,3),cex=1.3)
		dev.off()
		ev <- "event"
